...
@section('title', 'Nuevo TipoDocumento')
<!-- STYLES -->
        ...
        <form class="form_cliente" method="POST" action="{{route('document-types.store')}}">
            ...
            <div class="row">
                <div class="col-md-6 form-floating mb-1">
                    <input type="text" autocomplete="off" class="col form-control" id="codigo" name="codigo" value="{{ old('codigo') }}" placeholder="Código de Documento" required>
                    <label>Código</label>
                    <label>Código Documento</label>
                </div>
                <div class="col-md-6 form-floating mb-1">
                    <input type="text" autocomplete="off" class="col form-control" value="{{ old('nombre') }}" id="nombre" name="nombre" placeholder="Nombre" required>
                    <label>Nombre</label>
                </div>
            </div>
            <div class="col-md-12 form-floating mb-1">
                <input type="text" autocomplete="off" class="col form-control" value="{{ old('legajo') }}" id="legajo" name="legajo" placeholder="Firmantes">
                <label>Legajos de firmantes, separados por (,)</label>

            </div>

            <div class="form-group col-md-12 form-floating">
                <select name="empleado_visualiza" id="empleado_visualiza" class="form-input form-select select_width" data-Live-search="true">
                    <option value="1">Si</option>
                    <option value="0">No</option>
                </select>
                <label>Empleado puede ver</label>
            </div>


            <div class="form-group col-md-12 form-floating">
                <select name="regex" id="regex" class="form-input form-select select_width" data-Live-search="true">
                    <option value="!\d{8}+!">XXXXXXXX || MODELO DNI</option>
                    <option value="!\d{11}+!">XXXXXXXXXXX || MODELO CUIL SEGUIDO</option>
                    <option value="/\d{2}-\d{8}-\d/">XX-XXXXXXXX-X || MODELO CUIL SEPARADO</option>
                </select>
                <label>Regex de formato de documento en el documento</label>
            </div>

            <!-- <div class="form-group col-md-6 form-floating">
                     <select name="destino" id="destino" class="form-input form-select select_width" data-Live-search="true">
                         <option value="Todos">Todos</option>
                         <option value="Gerencia">Gerencia</option>
                         <option value="Sucursal">Sucursal</option>
                         <option value="Empleado">Empleado</option>
                     </select>
                     <label>Destino documento</label>
                 </div> -->

            <div class="form-group col-md-12 form-floating">
                <select name="tipo_documento" id="tipo_documento" class="form-input form-select select_width" data-Live-search="true">
            ...
            <div class="row">
                ...
                <div class="form-check col-md-3">
                    <label class="label">Requiere Firma</label>
                    <div class="form-check"><input class="form-check-input" checked type="radio" name="requiere_firma" id="requiere_firma" value="Empleador" { (old('requiere_firma') == 'Empleador') ? "checked" : "Empleador" }}><label class="form-check-label">Empleador</label>
                    </div>
                    <div class="form-check"><input class="form-check-input" type="radio" name="requiere_firma" id="requiere_firma" value="Empleado" { (old('requiere_firma') == 'Empleado') ? "checked" : "Empleado" }}><label class="form-check-label">Empleado</label>
                    </div>
                    <div class="form-check"><input class="form-check-input" type="radio" name="requiere_firma" id="requiere_firma" value="Ambos" {{ (old('requiere_firma') == 'Ambos') ? "checked" : "Ambos" }}><label class="form-check-label">Ambos</label>
                    </div>
                </div>

                <div class="form-check col-md-3">
                    <label class="label">Estado</label>
                    <div class="form-check"><input class="form-check-input" checked type="radio" name="status" id="status" value="Activo" {{ (old('status') == 'Activo') ? "checked" : "" }}><label class="form-check-label">Activo</label>
                    </div>
                ...
                <div class="form-check col-md-3">
                    <label class="label">Periodo
                        <!-- <input type="checkbox" name="periodo_checkbox" value="1" id="periodo"></label> -->
                        <div class="form-check"><input class="form-check-input periodo-check" type="radio" name="periodo" value="0" checked><label class="form-check-label">Mes anterior</label>
                        </div>
                        <div class="form-check"><input class="form-check-input periodo-check" type="radio" name="periodo" value="1"><label class="form-check-label">Mes actual</label>
                        </div>
                </div>

                <div class="form-check col-md-3">
                    <label class="label">Empleado Inicia Documento</label>
                    <div class="form-check"><input class="form-check-input" type="radio" name="inicia" id="inicia" value="Empleado" {{ (old('inicia_documento') == 'Empleado') ? "checked" : "" }}><label class="form-check-label">Si</label>
                    </div>
                    <div class="form-check"><input class="form-check-input" type="radio" name="inicia" id="inicia" checked value="Administrador" {{ (old('inicia_documento') == 'Administrador') ? "checked" : "" }}><label class="form-check-label">No</label>
                    </div>
                </div>

                <div class="col-md-12 form-floating mb-1">
                    <input type="text" autocomplete="off" class="col form-control" value="{{ old('empleadores_visual') }}" id="empleadores_visual" name="empleadores_visual" placeholder="empleadores_visual" required>
                    <label>Empleadores que pueden visualizar el tipo de documento poner legajos con comas, si pones 1 todos pueden ver</label>
                </div>
            </div>

            <br>
            <div class="col-12 mt-2">
                <label class="label subtitulo">Roles que Inician este Tipo de Documento - (Solo habilitado a partir de la 1.5)</label>
                <hr>
            </div>
            ...
            {{-- <div class="col">
               <ol class="switches">
                  ...
                  <li>
                    <input type="checkbox" name="roles[]" id="rol-{{$rol->id}}" value="{{$rol->id}}">
